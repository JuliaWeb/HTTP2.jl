...
"""
    HTTP2.Form(data; boundary=string(rand(UInt128), base=16))
...
[`HTTP2.Multipart`](@ref) type is provided to support setting the `filename`, `Content-Type`,
...
data = Dict(
    ...
    "file2" => HTTP2.Multipart("dog.jpeg", open("mydog.jpg"), "image/jpeg"),
)
body = HTTP2.Form(data)
headers = []
HTTP2.post(url, headers, body)
```
"""
...
"""
    HTTP2.Multipart(filename::String, data::IO, content_type=HTTP2.sniff(data), content_transfer_encoding="")

A type to represent a single multipart upload chunk for a file. This type would be used as the value in a
key-value pair when constructing a [`HTTP2.Form`](@ref) for a request body (see example below).
...
`Content-Type` will default to the result of the `HTTP2.sniff(data)` mimetype detection algorithm, whereas
`Content-Transfer-Encoding` will be left out if not specified.

# Examples
```julia
body = HTTP2.Form(Dict(
    "key" => HTTP2.Multipart("File.txt", open("MyFile.txt"), "text/plain"),
))
headers = []
HTTP2.post(url, headers, body)
```
...
function Base.show(io::IO, m::Multipart{T}) where {T}
    ...
    print(io, "HTTP2.Multipart($(join(items, ", ")))")
end
